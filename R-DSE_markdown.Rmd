...
preFile <- choose.files(default = wkDir, caption = "Select pre-depolarization file",
                        multi = FALSE)
postFile <- choose.files(default = wkDir, caption = "Select post-depolarization file",
                        multi = FALSE)

outDir <- choose.dir(default = paste0(wkDir,"/"), "Select output directory")

```

```{r run-dseR, warning = FALSE}
#run dseR to get data
allData <- dseR(preFile, 
              postFile,
              numSweeps = 60, preAvg = 15, postAvg = 5)
```
...
```{r, render-rmd, eval = FALSE, include = FALSE}
#this doesn't work, need to figure out how to make it work
